...
%
%
function MakeVideo_Npts_halloween( nPts, WhoIsCenter, WheelRadius, ...
  DecorativeBez,...
...
for i = 0:nTimes
  ...
  fill([-.2,.2,.2,-.2],[-1.1,-1.1,0,0],[97,138,61]/255,'EdgeColor','none')
  %plot(DecorativeBez(1,:),DecorativeBez(2,:),'Color',[.4 .4 .4],'LineWidth',2)
  fill(DecorativeBez(1,:),DecorativeBez(2,:),[255,117,24]/255,'EdgeColor','none')
  %
  ...
  %
  for q = 1:nCenters
    fill(RefWheelCtr(1,q)+circ(1,:),RefWheelCtr(2,q)+circ(2,:), 'white', 'EdgeColor', 'none','FaceAlpha',0.15); 
  ...
  scatter(RefBez(1),RefBez(2),10,'black','filled')
  %
  end
...
for tmp = 1:1
  ...
  fill(DecorativeBez(1,:),DecorativeBez(2,:),[255,117,24]/255,'EdgeColor','none')
  for p = 1:nPts
    fill(AllMarkerPos{p}(1,:),AllMarkerPos{p}(2,:),'black','EdgeColor','none')
    plot(AllMarkerPos{p}(1,CurrPts{p}),AllMarkerPos{p}(2,CurrPts{p}),'Color',[255,117,24]/255,'LineWidth',ExtraOpts.LineWidth)
  end
  %
end
