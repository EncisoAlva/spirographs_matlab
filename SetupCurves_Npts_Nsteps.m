...
% animation.
% New feature: it will create several points spanning fro the wheel center
% to given points at the wheel edge. This will result on a very specific
% type of animations.
%
% ---- INUPUT ------------------------------------------------------------
...
AllLocTime     = cell(1,nPts*nSteps);
AllWhCtrPos    = cell(1,nPts*nSteps);
AllMarkerPos   = cell(1,nPts*nSteps);
...
for j = 1:nSteps
  ...
  AllBezierPos{  (j-1)*nPts + i} = BezierPos;
  AllLocTime{    (j-1)*nPts + i} = LocTime;
  AllWhCtrPos{   (j-1)*nPts + i} = WhCtrPos;
  AllMarkerPos{  (j-1)*nPts + i} = MarkerPos;
  AllMarkerAngle{(j-1)*nPts + i} = MarkerAngle;
end
end
